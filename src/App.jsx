...
const toTime = ts => new Date(ts*1000).toLocaleTimeString(“en-GB”,{hour:“2-digit”,minute:“2-digit”});
...
const lg = LEAGUES.find(l=>l.id===f.league?.id)||{name:f.league?.name||“Football”,country:f.league?.country||””,flag:“⚽”,tier:2};
...
league:lg.name, country:lg.country, flag:lg.flag, tier:lg.tier,
time:toTime(f.fixture?.timestamp||dt.getTime()/1000),
date:dt.toISOString().split(“T”)[0],
timestamp:dt.getTime(),
o25, o15:Math.min(o25+22,92), btts:55,
hodd:2.10, dodd:3.40, aodd:3.50,
hForm:“WDWLW”, aForm:“LWWDL”,
hasRealOdds:false, bookmaker:””,
};
}
...
let bm=null;
for(const k of BM_ORDER){bm=ev.bookmakers?.find(b=>b.key===k);if(bm)break;}
...
const obj={ho,ao,dro,o25o,o15o,bm:bm.title};
const clean=s=>s.toLowerCase().replace(/\s+(fc|cf|sc|ac|as|utd|united|city|town|rovers|athletic|albion|county|palace|wanderers)$/,””).trim();
const k1=`${ev.home_team}|${ev.away_team}`.toLowerCase();
const k2=`${clean(ev.home_team)}|${clean(ev.away_team)}`;
map[k1]=obj; map[k2]=obj;
}
...
if(ch.includes(kh||””)&&ca.includes(ka||””)) return v;
}
return null;
}

function enrichWithOdds(match,od) {
if(!od||!od.ho) return match;
const rH=1/od.ho, rD=1/od.dro, rA=1/od.ao;
const tot=rH+rD+rA;
const hp=Math.round((rH/tot)*100), dp=Math.round((rD/tot)*100), ap=Math.round((rA/tot)*100);
const o25 = od.o25o>0 ? Math.round((1/od.o25o)*100*1.05) : match.o25;
const o15 = od.o15o>0 ? Math.round((1/od.o15o)*100*1.05) : match.o15;
return {
…match,
...
bookmaker:od.bm, hasRealOdds:true,
};
}
...
const LEG_WR = {3:62,4:54,5:45,6:38,7:31,8:25,9:20,10:17,11:14,12:11,13:9,14:7,15:6,16:5,20:3,25:1,30:1,40:0.3,50:0.1};

// ── QUANTITATIVE EV ENGINE ────────────────────────────────────
function formScore(f=“WDWLW”) {
const w=[1.0,0.9,0.8,0.7,0.6];
const ch=f.slice(-5).split(””).reverse();
let s=0,t=0;
ch.forEach((c,i)=>{const wt=w[i]||0.5;s+=wt*(c===“W”?1:c===“D”?0.4:0);t+=wt;});
return s/t;
}
...
lH:+lH.toFixed(2),lA:+lA.toFixed(2)
};
}
...
reasons.push(`xG H${pm.lH} A${pm.lA}`);
if(edge>0.08) reasons.push(`Strong edge`);
if(ev>0.1)   reasons.push(`+EV`);
return {id,tp:Math.round(tp*100),ip:Math.round(imp*100),edge,ev,odds:+odds.toFixed(2),reason:reasons.join(” · “)};
}
...
if(!fb) return null;
return {…fb,noValue:true,score:Math.max(Math.round(fb.tp*0.4),10),prob:fb.tp};
}

const best=cands[0];
const score=Math.min(Math.round((best.ev+1)*40+best.edge*60),99);
return {…best,noValue:false,score:Math.max(score,10),prob:best.tp};
}

// Legacy wrapper for market display grid
...
{m.hasRealOdds&&<span style={{background:“rgba(0,255,136,.15)”,color:”#00ff88”,fontSize:8,padding:“1px 5px”,borderRadius:99,fontWeight:800}}>📊 {m.bookmaker||“Live Odds”}</span>}
</div>
{/* Teams + prob */}
...
<span style={{fontSize:9,background:`rgba(240,192,64,.08)`,color:edgeCol,padding:“2px 6px”,borderRadius:99,fontWeight:700}}>
...
{[[“1 HOME”,m.hodd,Math.round((1/m.hodd)*100)],[“X DRAW”,m.dodd,Math.round((1/m.dodd)*100)],[“2 AWAY”,m.aodd,Math.round((1/m.aodd)*100)]].map(([l,o,p])=>(
...
{/* xG row */}
<div style={{display:“grid”,gridTemplateColumns:“1fr 1fr 1fr”,gap:5,marginBottom:10}}>
{[[“xG Home”,pm.lH],[“xG Away”,pm.lA],[“O2.5”,trueProb(m,“over25”)+”%”]].map(([l,v])=>(
...
<button onClick={()=>setOpen(!open)} style={{width:“100%”,background:“rgba(255,255,255,.02)”,border:“none”,borderTop:“1px solid rgba(255,255,255,.04)”,color:”#2a2a2a”,fontSize:9,cursor:“pointer”,padding:“8px”}}>
{open?“▲ HIDE MARKETS”:“▼ VIEW ALL MARKETS”}
</button>
{open&&(
...
Edge: {(evd.edge*100).toFixed(1)}% · EV: {evd.ev>0?”+”:””}{evd.ev.toFixed(2)} · {evd.odds}x
</div>
</div>
);
})}
...
if(!HAS_FIXTURES){setMatches(DEMO);setIsLive(false);setLoading(false);return;}
try {
const fixtures=await fetchFixtures();
if(fixtures.length===0){setMatches(DEMO);setIsLive(false);setStatus(“No fixtures found”);setLoading(false);return;}
...
parsed=parsed.map(m=>{const od=findOdds(oddsMap,m.home,m.away);return od?enrichWithOdds(m,od):m;});
const n=parsed.filter(m=>m.hasRealOdds).length;
setStatus(`${parsed.length} fixtures · ${n} with real odds`);
}catch{setStatus(`${parsed.length} fixtures`);}
}else{
setStatus(`${parsed.length} fixtures loaded`);
}
setMatches(parsed); setIsLive(true);
}catch(e){
setMatches(DEMO); setIsLive(false);
...
const combOdds=picks.reduce((a,p)=>a*(p.odds||p.prob?1/(p.prob/100):1),1);
const winChance=picks.reduce((a,p)=>a*(p.prob/100),1)*100;
const avgEV=picks.length?+(picks.reduce((a,p)=>a+p.ev,0)/picks.length).toFixed(3):0;
...
.map(m=>{const b=pickBest(m,allowed,minEdge,minOdds,maxOdds,usedMkts);if(b)usedMkts.push(b.id);return{…m,_b:b};})
...
ev:m._b.ev,edge:m._b.edge,odds:m._b.odds,
...
legs,result:“Pending”,odds:co.toFixed(2),
...
try{
  ...
  const res=await fetch("https://api.anthropic.com/v1/messages",{
    ...
    body:JSON.stringify({
      ...
      messages:[{role:"user",content:`You are a quantitative football betting analyst. This accumulator was built using a Poisson distribution model blended with real bookmaker odds. Selection criteria: EV maximisation, edge>threshold, diversity enforcement.\n\n${detail}\n\nCombined odds: ${co.toFixed(2)}x | Win prob: ${wc.toFixed(1)}% | Historical ${legs}-leg win rate: ${wr}% | Avg EV: ${avgEV>0?"+":""}${avgEV.toFixed(3)}\n\nAnalyse using EV principles:\n🎯 PICK ANALYSIS — edge source and why this market has value over bookmaker\n⚠️ RISK — where Poisson model assumptions could fail\n📊 PORTFOLIO — combined edge and market diversity\n✅ VERDICT — confidence /10, Kelly-adjusted stake (1-5%), one key insight\n\nBe sharp, quantitative, reference xG and edge numbers.`}]
    })
  });
  const data=await res.json();
  ...
  if(e.message==="NO_KEY") setAiText("⚠️ Add VITE_ANTHROPIC_KEY in Vercel → Environment Variables → Redeploy.");
  else setAiText("⚠️ "+e.message);
}
setAiLoad(false);
```
...
const copyAll=()=>{navigator.clipboard?.writeText(`🏆 WinSmart ${legs}-Leg Value Acca\n\n${pickTxt()}\n\n💰 Odds: ${picks.reduce((a,p)=>a*p.odds,1).toFixed(2)}x\n\nacca-ai.vercel.app`);setCopied(true);setTimeout(()=>setCopied(false),3000);};
const shareWA=()=>window.open(`https://wa.me/?text=${encodeURIComponent(`🏆 WinSmart ${legs}-Leg Value Acca\n\n${pickTxt()}\n\nacca-ai.vercel.app`)}`,”_blank”);
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div>
          ...
          <div style={{fontSize:9,color:"#2a2a2a",letterSpacing:.5}}>POISSON MODEL · EV ENGINE · REAL ODDS</div>
        </div>
      </div>
  ...
  <div style={{padding:"14px 12px 40px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12,animation:"up .3s ease"}}>
        ...
        {!isLive?(
          <div style={{background:"rgba(240,192,64,.06)",border:"1px solid rgba(240,192,64,.14)",borderRadius:12,padding:"11px 13px"}}>
            ...
            <button onClick={load} style={{padding:"5px 12px",borderRadius:8,border:"none",background:"rgba(240,192,64,.15)",color:"#f0c040",fontSize:10,fontWeight:700,cursor:"pointer"}}>{loading?"⏳ Loading...":"🔄 Load Live Data"}</button>
          </div>
        ):(
        ...
        <div style={{background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.05)",borderRadius:14,padding:14}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
            {[0,1,2,3,4,5,6].map(d=>(
              <button key={d} onClick={()=>toggleDay(d)} style={{padding:"6px 12px",borderRadius:10,border:"none",background:days.includes(d)?"#00ff88":"rgba(255,255,255,.06)",color:days.includes(d)?"#000":"#555",fontSize:11,fontWeight:800,cursor:"pointer"}}>{dayLabel(d)}</button>
            ))}
        ...
        <div style={{background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.05)",borderRadius:14,padding:14}}>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:12}}>
            {MARKETS.filter(m=>m.cat===mcat).map(m=>{
              ...
              return(
                <button key={m.id} onClick={()=>toggleMkt(m.id)} style={{padding:"6px 10px",borderRadius:8,border:"none",background:on?"rgba(0,255,136,.14)":"rgba(255,255,255,.04)",color:on?"#00ff88":"#555",fontSize:11,fontWeight:700,cursor:"pointer",outline:on?"1px solid rgba(0,255,136,.35)":"none"}}>
        ...
        <div style={{background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.05)",borderRadius:14,padding:14,display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              <div><div style={{fontSize:11,color:"#666",fontWeight:700}}>Min Edge</div><div style={{fontSize:9,color:"#333"}}>True prob − bookmaker implied</div></div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:8,color:"#1a1a1a"}}>0% — All picks</span><span style={{fontSize:8,color:"#1a1a1a"}}>15% — Value only</span></div>
          </div>

          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              <div><div style={{fontSize:11,color:"#666",fontWeight:700}}>Min Odds</div><div style={{fontSize:9,color:"#333"}}>Exclude below this price</div></div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:8,color:"#1a1a1a"}}>1.00 — Any</span><span style={{fontSize:8,color:"#1a1a1a"}}>3.00 — Higher risk</span></div>
          </div>

          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              <div><div style={{fontSize:11,color:"#666",fontWeight:700}}>Max Odds</div><div style={{fontSize:9,color:"#333"}}>Exclude high-risk outsiders</div></div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:8,color:"#1a1a1a"}}>1.5 — Safe only</span><span style={{fontSize:8,color:"#1a1a1a"}}>∞ — No limit</span></div>
          </div>
          ...
          <div style={{padding:"10px 12px",background:"rgba(0,255,136,.04)",border:"1px solid rgba(0,255,136,.1)",borderRadius:10}}>
            ...
            <div style={{fontSize:9,color:"#00ff8866",lineHeight:1.6}}>Poisson distribution + bookmaker blend · Selects highest EV market · dc12 blocked unless edge &gt;8% · Acca diversity enforced</div>
          </div>
        </div>
        ...
        {picks.length>0&&(
          <div style={{background:"linear-gradient(135deg,rgba(0,255,136,.07),rgba(0,180,80,.03))",border:"1px solid rgba(0,255,136,.18)",borderRadius:16,padding:16,animation:"up .3s ease"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:14}}>
              {[
                ["ODDS",picks.reduce((a,p)=>a*p.odds,1).toFixed(2)+"x","#fff"],
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <button onClick={copyAll} style={{width:"100%",padding:"12px",borderRadius:11,background:copied?"#00ff88":"rgba(0,255,136,.12)",border:"1px solid rgba(0,255,136,.28)",color:copied?"#000":"#00ff88",fontSize:12,fontWeight:800,cursor:"pointer"}}>
          ...
          <>
            <div style={{background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.05)",borderRadius:14,padding:14}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:12}}>
                {[["ODDS",picks.reduce((a,p)=>a*p.odds,1).toFixed(2)+"x","#fff"],["WIN %",winChance.toFixed(1)+"%","#00ff88"],["HIST WR",wr+"%","#f0c040"]].map(([l,v,c])=>(
                  <div key={l} style={{textAlign:"center"}}>
                    <div style={{fontSize:8,color:"#333"}}>{l}</div>
            ...
            <div style={{background:"rgba(167,139,250,.04)",border:"1px solid rgba(167,139,250,.14)",borderRadius:14,padding:14}}>
              ...
              {aiLoad?<div style={{fontSize:12,color:"#1a1a1a",animation:"shimmer 1.5s ease infinite"}}>Analysing EV model...</div>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={generate} style={{flex:1,padding:"13px",borderRadius:12,border:"none",background:"rgba(0,255,136,.08)",color:"#00ff88",fontSize:12,fontWeight:800,cursor:"pointer"}}>🔄 Regen</button>
    ...
    {tab==="stats"&&(
      <div style={{animation:"up .3s ease",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{background:"rgba(255,255,255,.025)",border:"1px solid rgba(255,255,255,.05)",borderRadius:14,padding:16}}>
          <div style={{fontSize:9,color:"#333",letterSpacing:1,marginBottom:14}}>ALL MARKETS RANKED BY WIN RATE</div>
