...
async function fetchFixtures() {
  ...
  return all.filter(f=>f.fixture?.status?.short==="NS").map(parseFixture).filter(Boolean).sort((a,b)=>a.timestamp-b.timestamp);
}
...
function parseFixture(f){
  ...
  const t=info.tier;
  const avgG=t===1?2.7:2.3;
  const o25=Math.round(38+(avgG-2)*16);
  return{
    id:f.fixture?.id, home:f.teams?.home?.name||"Home", away:f.teams?.away?.name||"Away",
    league:info.name, flag:info.flag, tier:t, time, date, timestamp:dt.getTime(),
    hp:46,dp:26,ap:28, btts:56, o15:Math.min(o25+22,92), o25,
    conf:t===1?80:66, hodd:2.15,dodd:3.40,aodd:3.50, ca:t===1?10.2:8.8, cr:3.8,
    hForm:"WWDLW", aForm:"LWWDL", venue:f.fixture?.venue?.name||"",
  };
}
...
function calcAI(m,id){
  const p=calcProb(m,id)/100,c=(m.conf||70)/100,f=(fScore(m.hForm)+fScore(m.aForm))/2;
  const v=Math.min(Math.max(p-(m.hodd?1/m.hodd:p),0)/0.15,1),t=m.tier===1?0.33:0.22;
...
export default function App(){
  ...
  async function generate(){
    ...
    try{
      ...
      const detail=np.map(p=>`${p.match.home} vs ${p.match.away} [${p.match.league}] — ${p.label} (${p.prob}%, AI ${p.score}/100)`).join("\n");
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":ANTHROPIC_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:`Football betting analyst. Analyse this ${legs}-leg accumulator:\n${detail}\n\nOdds:${odds.toFixed(2)}x | Win:${wc.toFixed(1)}% | Historical win rate:${wr}%\n\nWrite:\n🎯 PICK ANALYSIS — why each market\n⚠️ RISKS — key dangers\n✅ VERDICT — confidence/10, stake 1-5 units\n\nBe sharp and direct.`}]})});
      const data=await res.json();
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",color:C.text,maxWidth:480,margin:"0 auto"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:${C.dim};border-radius:99px}
        input[type=range]{-webkit-appearance:none;width:100%;background:transparent}
        input[type=range]::-webkit-slider-runnable-track{height:4px;background:${C.border};border-radius:99px}
        input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:18px;height:18px;border-radius:50%;background:${C.greenLt};margin-top:-7px}
